...
describe('/api/movers', () => {
  it('serves the mapped board, said to be the last close and not the running day', async () => {
    const res = await call({ board: 'gainers' }, respondWith(okBody));
  ...
  it('reads an empty board as a real answer, not as a failure', async () => {
    const res = await call({ board: 'losers' }, respondWith({ data: [] }));
    expect(res._status).toBe(200);
  ...
  it('refuses a repeated parameter rather than picking one', async () => {
    const res = await call({ board: ['gainers', 'losers'] }, respondWith(okBody));
    expect(res._status).toBe(400);
  ...
  it('caches a success for half an hour and a failure not at all', async () => {
    expect((await call({ board: 'gainers' }, respondWith(okBody)))._headers['Cache-Control']).toContain(
      's-maxage=1800',
    );
    expect(
      (await call({ board: 'gainers' }, respondWith({}, 403)))._headers['Cache-Control'],
    ).toBeUndefined();
  });
