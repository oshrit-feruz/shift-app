...
export type SessionResult =
  | { ok: true; userId: string; link: SnapTradeLink | null; store: LinkStore }
...
/**
 * Resolves the caller from their bearer token and reads their link.
 *
...
export async function resolveSession(req: ApiRequest, route: string): Promise<SessionResult> {
  ...
  try {
    return { ok: true, userId: who.userId, link: await readLink(store, who.userId), store };
  } catch (err) {
