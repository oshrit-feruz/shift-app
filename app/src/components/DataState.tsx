...
export function DataState<T>({
  ...
  if (state.status === 'loading') {
    ...
    return (
      <div
        ...
        style={{ textAlign: 'center', padding: '16px 0', fontSize: 19 }}
      >
        {t('data.loading')}
      </div>
    );
  ...
  if (state.status === 'unavailable') {
    return (
      ...
      >
        <span style={{ fontSize: 20 }}>{t('data.unavailable')}</span>
        ...
        {onRetry && (
          <Button variant="ghost" onClick={onRetry} alignSelf="center" fontSize={19}>
            {t('data.retry')}
          </Button>
        )}
      </div>
...
export function EmptyState({ children }: { children: ReactNode }) {
  return (
    <div style={{ textAlign: 'center', padding: '14px 0', color: 'var(--muted)', fontSize: 20 }}>
      {children}
    </div>
  );
}
