 ...
 *
 * TWO ENDPOINTS, TWO SURFACES:
 * - /api/beta/dashboard → the engine's OPEN POSITIONS (entry/current price).
 *   This backs the "open positions right now" card the product spec defines
 *   (honest empty state until the engine actually opens a position).
 * - /api/screener → today's ranked candidates; the BUY signals back a separate,
 *   clearly-labeled "today's screener output" card. Engine output is never
 *   presented as positions.
 *
...
/**
 * Map one raw dashboard row to a SatellitePosition.
 * Returns null when the row has no usable ticker.
 */
...
export async function fetchSatellitePositions(
  ...
  try {
    const res = await fetchImpl(RECOVERY_DETECTOR_POSITIONS_URL, {
