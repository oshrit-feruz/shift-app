...
export function ConnectionsScreen(_: ScreenProps) {
  ...
  return (
    <div className="anim-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
      ...
      <Card padding="4px 0" gap={0}>
        {LINKED.map((c, i) => (
          ...
          >
            ...
            >
              <Num size={20}>{c.value}</Num>
              <Tag variant="accent" fontSize={17}>
                {t('connScreen.live')}
              </Tag>
            </span>
          </div>
      ...
      <Card padding="4px 0" gap={0}>
        <CardTitle>
          <span style={{ display: 'block', padding: '9px 13px 2px', fontSize: 21 }}>
            {t('connScreen.add')}
          </span>
        </CardTitle>
        <InstitutionRows />
      ...
      <Card padding={13} gap={7}>
        ...
        <Button variant="secondary" block fontSize={19} minHeight={40} onClick={() => setNewPfOpen(true)}>
