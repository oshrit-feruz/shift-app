  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="text-muted" style={{ fontSize: 19, fontWeight: 600, letterSpacing: 0.4 }}>
            {t('auth.kicker')}
          </span>
          <h1 style={{ margin: 0, fontFamily: 'var(--font-heading)', fontSize: 32, lineHeight: 1.2 }}>
            {t('auth.title')}
          </h1>
          <p className="text-muted" style={{ margin: '0 0 18px', fontSize: 20, lineHeight: 1.5 }}>
            {t('auth.sub')}
          </p>
          ...
          {!isAppleEnabled && !notConfigured && (
            <span className="text-muted" style={{ fontSize: 18.5, textAlign: 'center' }}>
              {t('auth.appleSoon')}
            </span>
          )}
          ...
          {notConfigured && (
            <span className="text-muted" style={{ fontSize: 18.5, textAlign: 'center', lineHeight: 1.5 }}>
          ...
          {signInError && (
            ...
            >
              <span style={{ fontSize: 20 }}>{t('auth.errorTitle')}</span>
              <span className="text-muted" style={{ fontSize: 18.5, lineHeight: 1.5 }}>
                {signInError[language]}
              </span>
              <Button variant="ghost" onClick={clearSignInError} alignSelf="center" fontSize={19}>
                {t('auth.retry')}
              </Button>
            </div>
          )}
