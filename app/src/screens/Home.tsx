    ...
    >
      ...
      {beg && (
        ...
        >
          {(pfs) => {
            ...
            return (
              <Card padding={15} gap={0}>
                <div style={{ fontSize: 18, opacity: 0.75, fontWeight: 600 }}>
                  {t("home.pfToday")}
                </div>
                <div
                  style={{
      ...
      {setup.showBanner && (
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ flex: 1 }} />
            <span style={{ color: "var(--color-accent-200)", fontSize: 14 }}>
              {t("setup.resume")} ›
            </span>
          </div>
          <ProgressTrack
            pct={setup.pct}
            label={t("setup.stepOf", { n: setup.stepLabel })}
          />
        </Card>
      )}
      ...
      {beg && (
        ...
        >
          ...
          <span style={{ flex: 1 }}>
            <span style={{ display: "block", fontSize: 16.5 }}>
              {t("home.startHere")}
            </span>
            <span
              className="text-muted"
              style={{ display: "block", fontSize: 14.5, marginTop: 2 }}
            >
              {t("home.startHereSub")}
            </span>
          </span>
      ...
      <Card padding="13px 13px 4px" gap={6}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          ...
          <Button
            variant="ghost"
            fontSize={13}
            onClick={() => dispatch({ type: "go", screen: "watch" })}
          >
            {t("home.seeAll")}
          </Button>
        </div>
              ...
              {syms.slice(0, beg ? 4 : 6).map((x) => (
                <ListRow
                  ...
                  subtitle={
                    beg
                      ? x.plain[language]
                      : `${x.marketCap} · P/E ${x.pe.toFixed(1)}`
                  }
                  right={
                    <RowValues
                      main={money(x.price)}
          ...
          return (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <Tag variant="neutral">
                  {String(
                    page.truncated ? page.totalAvailable : page.rows.length,
                  )}
                </Tag>
              </div>
              <DemoBanner />
              {next.length === 0 ? (
                ...
                next.map((e) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      dispatch({ type: "openStock", ticker: e.ticker })
                    }
                    style={{
                      ...ROW_BUTTON_STYLE,
                      padding: "7px 0",
