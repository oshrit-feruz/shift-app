...
const OPEN = {
  ...
  he: [
    ...
    [
      'מה צריך להכין',
      'הפתיחה עצמה לוקחת כרבע שעה אונליין, אחרי שארבעת הדברים האלה מוכנים. אפשר לסמן כל אחד כשהוא בידיים.',
    ],
    ['איזה סוג חשבון', 'לכל ברוקר יש חשבון רגיל, ובדרך כלל גם חשבון אחד לפחות עם הטבת מס.'],
    [
      'להעביר כסף פנימה',
      'כדאי להתחיל בסכום שלא יחסר לך. אפשר להוסיף בכל רגע, וההעברה הראשונה בעיקר מוכיחה שהצינור עובד.',
    ],
...
const KINDS = {
  ...
  he: [
    ...
    ['app', 'אפליקציית השקעות', 'הפתיחה הכי מהירה, הבחירה הכי קטנה. כדאי לבדוק מה העלות כשמוכרים.'],
  ],
} as const;
...
const ACCTS = {
  ...
  he: [
    ...
    [
      'tax',
      'חשבון עם הטבת מס',
      'מס נמוך או נדחה, בתמורה לכללים לגבי מתי אפשר להוציא את הכסף. שווה לשאול את הברוקר לאיזה חשבון יש זכאות.',
    ],
  ],
} as const;
      ...
      {step === 3 && (
        <Card padding={16} gap={10}>
          ...
          <p className="text-muted" style={{ fontSize: 'var(--text-body)', margin: 0 }}>
            {language === 'he'
              ? 'העברה בנקאית נכנסת בדרך כלל בתוך יום עד שלושה ימי עסקים. כסף שיושב בחשבון הוא עדיין שלך ועדיין לא מושקע, עד שקונים בו משהו.'
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <Card padding={14} gap={6} style={{ marginTop: 2 }}>
            ...
            <p className="text-muted" style={{ fontSize: 'var(--text-body)', margin: 0, lineHeight: 1.5 }}>
              {language === 'he'
                ? 'הפקודה מתבצעת בשניות בשעות המסחר. היא תופיע בפוזיציות, ומשם התפקיד היחיד הוא לא לגעת.'
